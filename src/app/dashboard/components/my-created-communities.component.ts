  ...
  template: `
    <div>
      ...
      <div class="flex items-center justify-between px-4 mb-2">
        <p class="text-xs font-semibold uppercase text-slate-500">My Created Communities</p>
        <button
...
export class MyCreatedCommunitiesComponent {
  ...
  readonly communities = this.communityService.myCreatedCommunities;
  readonly isLoading = this.communityService.myCreatedCommunitiesLoading;
  readonly isEmpty = this.communityService.isMyCreatedCommunitiesEmpty;
  readonly error = this.communityService.myCreatedCommunitiesError;

  constructor() {
    console.log('[MyCreatedCommunitiesComponent] Component initialized');
    
    // Load communities on init
    effect(() => {
      console.log('[MyCreatedCommunitiesComponent] Effect triggered - loading created communities...');
  ...
  toggleCollapsed(): void {
    ...
    console.log('[MyCreatedCommunitiesComponent] toggleCollapsed called - new state:', newState);
    this.isCollapsed.update((value: boolean) => !value);
  }

  navigateToCreate(): void {
    console.log('[MyCreatedCommunitiesComponent] navigateToCreate called');
