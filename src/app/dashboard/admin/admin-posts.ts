        ...
        @if (loading()) {
          ...
          @for (post of posts(); track post.id) {
            <tr class="hover:bg-slate-50 transition-colors">
              <td class="px-5 py-3.5 font-semibold text-slate-800 max-w-xs">
                <p class="truncate max-w-[200px]">{{ post.title }}</p>
              </td>
              <td class="px-5 py-3.5 text-slate-500 whitespace-nowrap">{{ post.authorFullName }}</td>
    ...
    <div class="bg-white rounded-2xl shadow-xl max-w-md w-full p-6">
      ...
      <div class="flex gap-3">
        <button (click)="cancelDelete()"
          class="flex-1 px-4 py-2 border border-slate-200 text-slate-700 rounded-xl font-semibold hover:bg-slate-50">Cancel</button>
        <button (click)="executeDelete()"
          class="flex-1 px-4 py-2 bg-red-600 text-white rounded-xl font-semibold hover:bg-red-700">Delete</button>
      </div>
    </div>
  </div>
}
...
export class AdminPosts implements OnInit {
  ...
  load() {
    ...
    const req$ = filter === 'ALL'
      ? this.postFacade.getAll({ page: this.page(), size: this.size })
      : this.postFacade.getByStatus(filter, this.page(), this.size);
    ...
    req$.subscribe({
      ...
      error: err => {
        console.error('[AdminPosts] load error:', err);
        this.loading.set(false);
      }
    });
  }
  ...
  statusClass(status: string): string {
    ...
    switch (status) {
      ...
      case 'Flagged':  return base + 'bg-red-50 text-red-500 ring-red-200';
