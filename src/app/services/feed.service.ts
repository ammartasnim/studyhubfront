...
export class FeedService {
  ...
  readonly comments = computed(() => {
    ...
    this.commentStates().forEach((state, postId) =>
      result.set(postId, state.items)
    );
    return result;
  });
  ...
  readonly commentsLoading = computed(() => {
    ...
    this.commentStates().forEach((state, postId) => {
      if (state.loading) result.add(postId);
    });
    return result;
  });
  ...
  readonly commentHasMore = computed(() => {
    ...
    this.commentStates().forEach((state, postId) =>
      result.set(postId, state.hasMore)
    );
    return result;
  });
...
async loadFeed(): Promise<void> {
  ...
  try {
    ...
    this.totalPosts.set(result.totalItems); // ← added here

    this.hasMore.set(
      result.items.length >= FEED_PAGE_SIZE &&
  ...
  async addComment(postId: number, content: string): Promise<void> {
    const comment = await firstValueFrom(
      this.commentFacade.create({ content, postId })
    );
...
async deleteComment(postId: number, commentId: number): Promise<void> {
  ...
  // Auto-load next page if we deleted down to fewer than page size and more exist
  ...
  async loadReplies(commentId: number): Promise<void> {
    ...
    try {
      const res = await firstValueFrom(
        this.commentFacade.getReplies(commentId, 0, 5)
      );

      this.replyStates.update(m =>
        new Map(m).set(commentId, res.items ?? [])
      );
    } finally {
      this.replyLoading.update(s => {
        const n = new Set(s);
        n.delete(commentId);
        return n;
      });
  ...
  async addReply(commentId: number, content: string): Promise<void> {
    const reply = await firstValueFrom(
      this.commentFacade.createReply(commentId, { content })
    );

    this.replyStates.update(m => {
      const map = new Map(m);
      const list = map.get(commentId) ?? [];
      map.set(commentId, [...list, reply]);
      return map;
    });
  }
  ...
  async checkCommunities(): Promise<void> {
    try {
      const result = await firstValueFrom(
        this.communityFacade.getMy({ size: 1 })
      );
...
async deleteReply(commentId: number, replyId: number): Promise<void> {
  ...
  this.replyStates.update(m => {
    const newMap = new Map(m);
    const replies = newMap.get(commentId) ?? [];
    newMap.set(commentId, replies.filter(r => r.id !== replyId));
    return newMap;
  });
}
